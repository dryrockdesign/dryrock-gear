...
export function makeHelicalSTL(p, teeth, x) {
  ...
  // apply progressive twist around Z proportional to depth
  const pos = g.attributes.position;
  for (let i=0;i<pos.count;i++){
    const x0=pos.getX(i), y0=pos.getY(i), z0=pos.getZ(i);
    const frac = z0 / b;                 // 0..1 along the face
    const ang = totalTwist * frac;
...
export function clampHelical(raw) {
  ...
  return {
    module:n(raw.module,0.3,10,2), pa:n(raw.pa,10,30,20), res:Math.round(n(raw.res,6,40,16)),
    beta:n(raw.beta,1,45,20),                       // helix angle (deg)
    fillet:n(raw.fillet,0,0.45,0.38), fw:n(raw.fw,1,60,12),
    bore:n(raw.bore,0,60,6), fit:n(raw.fit,0,2,0.2),
    boreType:['round','d','key','hex','none'].includes(raw.boreType)?raw.boreType:'round',
    teeth:Math.round(n(raw.teeth,5,250,24)), teeth2:Math.round(n(raw.teeth2,5,300,48)),
